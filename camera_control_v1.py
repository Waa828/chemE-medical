...
class CameraControl:
    def __init__(self, digit_cam_id=0, color_cam_id=1, SCREEN_HEIGHT=680, SCREEN_WIDTH=840):
        ...
        # color detection
        self.lower_hsv = [67, 88, 83]
        self.upper_hsv = [67, 89, 100]
    ...
    def start_recording(self, filename):
        ...
        # 初始化 HSV Excel 记录
        self._hsv_data = []
        self._hsv_filename = filename.replace('.avi', '_hsv.xlsx')
    ...
    def stop_recording(self):
        ...
        if self._video_writer:
            ...
            self._video_writer = None
        if self._hsv_data and self._hsv_filename:
            df = pd.DataFrame(self._hsv_data, columns=['Time (s)', 'H (0-360)', 'S (0-100)', 'V (0-100)'])
