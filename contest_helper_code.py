...
class DigitContestRunner(_ContestBase):
    ...
    def _contest_loop(self):
        ...
        while self._running:
            ...
            if dot_in_center and nn_input_img is not None:
                pred = self._model.predict(nn_input_img)
                self._pred_lbl.value = f"Predicted Digit: {pred}"
                pred_history.pop(0)
                pred_history.append(pred)

                target = self._target_input.value
                valid_entries = [p for p in pred_history if p != -1]
                ratio = valid_entries.count(target) / len(valid_entries) if valid_entries else 0.0
                if ratio > CONFIDENCE:
                    print("stop")
                    self._motor.emergency_stop()
                    pos = self._motor.read_physical_position()
                    with self._output:
                        print(f"✓ Target digit {target} detected (confidence {ratio:.2f}) → motor stopped.")
                        print(f"  Time: {elapsed:.2f} s  |  Position: {pos:.2f} mm")
                    self._finish(True, elapsed, pos)
                    return
            else:
                if not dot_in_center:
                    self._pred_lbl.value = "Predicted Digit: (no card)"
                else:
